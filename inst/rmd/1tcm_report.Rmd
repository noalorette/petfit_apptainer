...
This report summarises the 1TCM model fitting step.
...
if (!is.null(config$Models)) {
  if (model_number == "Model 1") {
    model_config <- config$Models$Model1
  } else if (model_number == "Model 2") {
    model_config <- config$Models$Model2
  } else if (model_number == "Model 3") {
...
if (!is.null(model_config) && model_config$type == "1TCM") {
  ...
  config_items <- list(
    "Model Type" = "1TCM (One Tissue Compartment Model)",
  ...
  kable(config_df, caption = "1TCM Model Configuration")
...
# Verify we have a valid 1TCM configuration
has_1tcm_config <- !is.null(model_config) && model_config$type == "1TCM"

# Exit early if not a 1TCM configuration
if (!has_1tcm_config) {
  cat("Invalid or missing 1TCM configuration. This report should not have been generated.")
  knitr::knit_exit()
}
```
...
Now we proceed to the actual 1TCM model fitting using all available regional TACs.
...
```{r model-fitting}
# Safe 1TCM function to handle errors
...
model_res <- model_data %>% 
  ungroup() %>% 
  mutate(par = map(fit_1tcm, "par")) %>% 
  select(-fit_1tcm, -tacs, -input)
```
...
par_se_table <- model_data %>% 
  ungroup() %>% 
  mutate(par.se = map(fit_1tcm, "par.se")) %>% 
  select(-fit_1tcm, -tacs, -input) %>% 
  select(-output_stem) %>% 
  select(where(~ n_distinct(.x) > 1)) %>% 
  select(-any_of(c("vB", "inpshift"))) %>% 
...
gof_table <- model_data %>% 
  ungroup() %>% 
  mutate(gof = map(fit_1tcm, ~tibble(
    AIC = AIC(.x$fit),
    BIC = BIC(.x$fit),
    RSS = RSS(.x$fit),
...
savepar_pet <- savepar_pet %>% 
  ...
  mutate(jsondat = map(kinpars, ~list(
      Description = "One tissue compartmental model fit using nonlinear least squares estimation.",
      ModelName = "1TCM",
...
savefits_pet <- savefits_pet %>% 
  ...
  mutate(jsondat = map(fitvals, ~list(
      Description = "One tissue compartment model fitted values.",
      ModelName = "1TCM",
      AdditionalModelDetails = str_glue("vB source: {config_items$`vB Parameter Source`}. Blood delay source: {delay_source}"),
