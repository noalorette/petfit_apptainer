---
title: "Irreversible 2TCM Model Fitting Report"
output: 
  html_document:
    toc: true
    toc_depth: 2
...
This report summarises the irreversible 2TCM model fitting step.

# Analysis Configuration

**Model:** `r params$model_number` - 2TCM_irr (Two Tissue Compartment Model with Irreversible Binding)
...
if (!is.null(model_config) && model_config$type == "2TCM_irr") {
  # Create configuration table
  config_items <- list(
    "Model Type" = "Irreversible 2TCM (Two Tissue Compartment Model with Irreversible Binding)",
  ...
  kable(config_df, caption = "Irreversible 2TCM Model Configuration")

} else {
  cat("2TCM_irr model configuration not found or incomplete.")
}
```

```{r setup-conditions, echo=FALSE}
# Verify we have a valid 2TCM_irr configuration
has_2tcm_irr_config <- !is.null(model_config) && model_config$type == "2TCM_irr"

# Exit early if not a 2TCM_irr configuration
if (!has_2tcm_irr_config) {
  cat("Invalid or missing 2TCM_irr configuration. This report should not have been generated.")
  knitr::knit_exit()
}
```
...
# 2TCM_irr Model Fitting

Now we proceed to the actual 2TCM_irr model fitting using all available regional TACs. Note that k4 is fixed to 0 in this irreversible binding model.
...
k2_lower <- model_config$k2$lower %||% 0.0001
k2_upper <- model_config$k2$upper %||% 0.5

k3_start <- model_config$k3$start %||% 0.1
k3_lower <- model_config$k3$lower %||% 0.0001
k3_upper <- model_config$k3$upper %||% 0.5

# Note: k4 is fixed to 0 in 2TCM_irr model (irreversible binding)

vB_start <- model_config$vB$start %||% 0.05
vB_lower <- model_config$vB$lower %||% 0.01
vB_upper <- model_config$vB$upper %||% 0.1
...
```{r model-fitting}
# Safe 2TCM_irr function to handle errors
safe_twotcm_irr <- possibly(twotcm_irr, otherwise = NA)
...
model_res <- model_data %>%
  ungroup() %>%
  mutate(par = map(fit_2tcm_irr, "par")) %>%
  select(-fit_2tcm_irr, -tacs, -input)
```
...
par_se_table <- model_data %>%
  ungroup() %>%
  mutate(par.se = map(fit_2tcm_irr, "par.se")) %>%
  select(-fit_2tcm_irr, -tacs, -input) %>%
  select(-output_stem) %>%
  select(where(~ n_distinct(.x) > 1)) %>%
  select(-any_of(c("vB", "inpshift"))) %>%
...
gof_table <- model_data %>%
  ungroup() %>%
  mutate(gof = map(fit_2tcm_irr, ~tibble(
    AIC = AIC(.x$fit),
    BIC = BIC(.x$fit),
    RSS = RSS(.x$fit),
...
par_table_long <- par_table %>%
  pivot_longer(cols=c(K1, k2, k3, Ki, Vnd, lambdak3),
               names_to = "Parameter",
               values_to = "Value")
...
savepar_pet <- savepar_pet %>% 
  ...
  mutate(jsondat = map(kinpars, ~list(
      Description = "Two tissue compartmental model with irreversible binding fit using nonlinear least squares estimation.",
      ModelName = "2TCM_irr",
...
savefits <- model_data %>% 
  filter(success) %>% 
  mutate(fitvals = map(fit_2tcm_irr, ~{
...
savefits_pet <- savefits_pet %>% 
  ...
  mutate(jsondat = map(fitvals, ~list(
      Description = "Two tissue compartment model with irreversible binding fitted values.",
      ModelName = "2TCM_irr",
      AdditionalModelDetails = str_glue("vB source: {config_items$`vB Parameter Source`}. Blood delay source: {delay_source}"),
