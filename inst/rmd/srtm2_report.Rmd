---
title: "SRTM2 Model Fitting Report"
output:
  html_document:
    toc: true
    toc_float: true
...
This report summarises the model fitting step using the Simplified Reference Tissue Model 2 (SRTM2) (Wu & Carson, 2002).

> Wu Y, Carson RE. Noise reduction in the simplified reference tissue model for neuroreceptor functional imaging. Journal of Cerebral Blood Flow & Metabolism. 2002;22:1440-1452.

# Analysis Configuration

**Model:** `r params$model_number` - SRTM2 (Simplified Reference Tissue Model 2)
...
if (!is.null(model_config) && model_config$type == "SRTM2") {
  # Create configuration table
  config_items <- list(
    "Model Type" = "SRTM2 (Simplified Reference Tissue Model 2)",
  ...
  kable(config_df, caption = "SRTM2 Model Configuration")

} else {
  cat("SRTM2 model configuration not found or incomplete.")
}
```

```{r setup-conditions, echo=FALSE}
# Verify we have a valid SRTM2 configuration
has_srtm2_config <- !is.null(model_config) && model_config$type == "SRTM2"

# Exit early if not a SRTM2 configuration
if (!has_srtm2_config) {
  cat("Invalid or missing SRTM2 configuration. This report should not have been generated.")
  knitr::knit_exit()
}
```
...
Now we proceed to the actual SRTM2 model fitting using all available regional TACs.
...
# Safe SRTM2 function to handle errors
safe_srtm2 <- possibly(srtm2, otherwise = NA)
...
model_res <- model_data %>%
  ungroup() %>%
  mutate(par = map(fit_srtm2, "par")) %>%
  select(-fit_srtm2, -tacs)
```
...
par_se_table <- model_data %>%
  ungroup() %>%
  mutate(par.se = map(fit_srtm2, "par.se")) %>%
  select(-fit_srtm2, -tacs) %>%
  select(-output_stem) %>%
  select(where(~ n_distinct(.x) > 1)) %>%
  select(-k2prime) %>% 
...
gof_table <- model_data %>%
  ungroup() %>%
  mutate(gof = map(fit_srtm2, ~tibble(
    AIC = AIC(.x$fit),
    BIC = BIC(.x$fit),
    RSS = RSS(.x$fit),
...
savepar_pet <- savepar_pet %>%
  ...
  mutate(jsondat = map(kinpars, ~list(
      Description = "Simplified Reference Tissue Model 2 fit using nonlinear least squares estimation.",
      ModelName = "SRTM2",
      AdditionalModelDetails = str_glue("Reference tissue model - no blood data required. k2prime source: {k2prime_source_desc}")))) %>%
...
savefits_pet <- savefits_pet %>%
  ...
  mutate(jsondat = map(fitvals, ~list(
      Description = "Simplified Reference Tissue Model 2 fitted values.",
      ModelName = "SRTM2",
      AdditionalModelDetails = str_glue("k2prime source: {k2prime_source_desc}"),
